const konjunktionenData = window.LWG_KONJUNKTIONEN_DATA || {};
const { connectors, categories, categoriesEn, uiCopy, quizQuestions } = konjunktionenData;
...
function renderReference() {
  ...
  <div class="connector-meta">
    ...
    <span class="connector-pill">${ui('noVerbFinal')}</span>
  </div>
...
function finishQuiz() {
  ...
  const excellentThreshold = Math.ceil(quizState.shuffled.length * 0.8);
  const goodThreshold = Math.ceil(quizState.shuffled.length * 0.6);
  const heading =
    quizState.score >= excellentThreshold
      ? resultSet[0]
      : quizState.score >= goodThreshold
        ? resultSet[1]
        : resultSet[2];
  const message =
    quizState.score >= excellentThreshold
    ...
    .map(([category, items]) => {
      ...
      const rows = items
        .map((item) => {
          ...
          return `<div class="cs-row konjunktion-row">
