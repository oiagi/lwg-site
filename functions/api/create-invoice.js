...
export const onRequestPost = withErrorHandling(async ({ request, env }) => {
  ...
  if (company_id && student_id)
    return errorResponse('Provide either company_id or student_id, not both', 400);
  if (!session_ids || !session_ids.length)
    return errorResponse('At least one session is required', 400);
  ...
  try {
    ...
    if (company_id) {
      ...
      const stuRes = await fetch(`${SUPABASE_URL}/rest/v1/students?id=eq.${student_id}&select=*`, {
        headers: H,
      });
    ...
    const linesRes = await fetch(`${SUPABASE_URL}/rest/v1/invoice_lines`, {
      method: 'POST',
      headers: { ...H, Prefer: 'return=representation' },
